...
export class AvailabilityCardComponent {
  ...
  // Add these methods in your component class
getQuotaCardClass(status: string): { [key: string]: boolean } {
  return {
    'border-success': status === 'Available' || status === 'CURR_AVBL' || status === 'RAC',
    'border-warning': status?.indexOf('WL') > 0,
    'border-danger': status === 'REGRET' || status === 'NOT AVAILABLE' || status === 'CLASS NOT EXIST',
  };
}
...
getAvailabilityClass(status: string): { [key: string]: boolean } {
  return {
    ...
    'text-danger': status === 'REGRET' || status === 'NOT AVAILABLE' || status === 'CLASS NOT EXIST',
  };
}
